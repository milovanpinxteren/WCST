...
from scoutingtool.models import Defense


class DefenseFilter():
    def filter_players(self, criterium):
        avg = Defense.objects.aggregate(Avg(criterium['field']))[criterium['field'] + '__avg']
        if criterium['value'] == '0': #no preference
            filtered_id_list = Defense.objects.all()
        if criterium['value'] == '1': #low amount
            filter_dict = {criterium['field'] + '__lte': avg}
            filtered_id_list = Defense.objects.filter(**filter_dict).values_list('player_id', flat=True)
            ...
            filtered_id_list = Defense.objects.filter(**filter_dict).order_by(ordering).values_list('player_id', flat=True)[:25]
