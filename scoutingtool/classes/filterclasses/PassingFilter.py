...
from scoutingtool.models import Passing


class PassingFilter():
    def filter_players(self, criterium):
        avg = Passing.objects.aggregate(Avg(criterium['field']))[criterium['field'] + '__avg']
        if criterium['value'] == '0': #no preference
            filtered_id_list = Passing.objects.all()
        if criterium['value'] == '1': #low amount
            filter_dict = {criterium['field'] + '__lte': avg}
            filtered_id_list = Passing.objects.filter(**filter_dict).values_list('player_id', flat=True)
            ...
            filtered_id_list = Passing.objects.filter(**filter_dict).order_by(ordering).values_list('player_id', flat=True)[:25]
