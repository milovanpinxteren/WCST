...
class PlayerInfoDictMaker:
    def make_dict(self, tag, resultset_counter, player_info_resultset, player_info_dict):
        ...
        if tag.text == 'Geboorteplaats:':
            player_info_dict['Geboorteplaats'] = player_info_resultset[resultset_counter + 1].text
        ...
        if tag.text == 'Nationaliteit:':
            player_info_dict['Nationaliteit'] = player_info_resultset[resultset_counter + 1].text
        if tag.text == 'Positie:':
            player_info_dict['Positie'] = player_info_resultset[resultset_counter + 1].text
        if tag.text == 'Voet:':
            player_info_dict['Voet'] = player_info_resultset[resultset_counter + 1].text
        ...
        if tag.text == 'Contract tot en met:':
            player_info_dict['Contract tot en met'] = player_info_resultset[resultset_counter + 1].text
        if tag.text == 'Positie:':
            player_info_dict['Positie'] = player_info_resultset[resultset_counter + 1].text
        resultset_counter += 1